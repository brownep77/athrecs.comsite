...
BEGIN
 ...
 IF b.status<>'ready' OR b.payload_hash<>'84b71b864514fc549f8c578473b9ad2fe6c92e0b58d2936247b476777262a2a0' OR b.validation_summary->>'invalidRows'<>'0' THEN RAISE EXCEPTION 'Only this validated ready batch may publish'; END IF;
 ...
 IF EXISTS(SELECT 1 FROM jsonb_array_elements(b.payload->'editions') d JOIN events e ON e.slug=d->>'eventSlug' JOIN editions ed ON ed.event_id=e.id AND ed.event_date=(d->>'date')::date AND ed.distance_code=d->>'distance') THEN RAISE EXCEPTION 'Edition appeared after validation'; END IF;
 ...
 FOR x IN SELECT value FROM jsonb_array_elements(b.payload->'events') LOOP
  ...
  IF event_id_value IS NULL THEN
   ...
   VALUES(x->>'slug',x->>'name',x->>'sport',x->>'country',coalesce(x->>'county',''),coalesce(x->>'city',''),coalesce(x->>'area',''),coalesce(x->>'surface','Road'),coalesce(x->>'summary',''),coalesce(x->>'description',''),coalesce(x->>'organiser',''),coalesce(x->>'website',''),false) RETURNING id INTO event_id_value;
  END IF;
 ...
 END LOOP;
 summary_value:=jsonb_build_object('eventsUpserted',184,'newEvents',77,'editionsUpserted',234,'entryOptionsUpserted',0,'revisionId',rev);
