...
The bounded batch was validated and published on 10 September 2026 after an authorised production duplicate check. This repository record matches the published values, including the entry-link verification date. No runtime code, seed version, workflow, domain or production setting is changed.
...
The normal staff stage → validate → review checks were completed before publication. The transaction-time checks found no conflicting event slug, historical slug, event reference or event/date/distance natural key.

## Production publication receipt

- Source key: `runrecs:bucharest-half-marathon:2027:2026-09-08`
- Batch ID: `ac91e3f6-dac1-4ee3-ba5b-4d62b436f1bc`
- Published at: `2026-09-10 06:43:07.248094+00`
- Revision: `16` (previous revision `15`)
- Event ID: `4375680`
- Half-marathon edition ID: `108804041`
- 10K edition ID: `108804042`
- Validation: 0 invalid rows / 0 errors

The production transaction inserted one event, two editions, two verified primary entry options and three audit snapshots. An independent read-only query confirmed exactly one matching May event and one record for each event/date/distance key. The separate April race and October Bucharest marathon records were not changed.

## Validation performed

Local structural validation passes for the recorded payload: one event, two editions, valid expected dates and weekdays, correct event references, expected official source and entry URLs, and zero duplicate event/date/distance keys within the batch. Live database validation and publication are documented in the receipt above.
