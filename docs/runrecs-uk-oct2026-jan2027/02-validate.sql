...
BEGIN
 IF jsonb_array_length((SELECT payload->'events' FROM catalogue_import_batches WHERE id='c7f2ccc0-4b61-579c-8f36-09e79e707459'))>75 THEN RAISE EXCEPTION 'Event batch exceeds 75'; END IF;
 SELECT * INTO STRICT b FROM catalogue_import_batches WHERE id='c7f2ccc0-4b61-579c-8f36-09e79e707459' FOR UPDATE;
 IF b.status <> 'staged' OR b.payload_hash <> '19b32727196acd520f149ce6245dbb51bdc9d5b11472aacd5e8d3df865a0f15b' THEN RAISE EXCEPTION 'Unexpected batch state or hash'; END IF;
 IF (SELECT count(*) FROM catalogue_staged_rows WHERE batch_id=b.id) <> 221 THEN RAISE EXCEPTION 'Staged row count differs'; END IF;
 ...
 IF n<>44 THEN RAISE EXCEPTION 'New-event count changed: %',n; END IF;
 UPDATE catalogue_import_batches SET status='ready',validated_at=now(),validation_summary=jsonb_build_object('events',75,'editions',146,'newEvents',44,'invalidRows',0,'errors','[]'::jsonb,'checks',jsonb_build_array('scope','source URLs','canonical names and aliases','date-distance keys','monthly recurrence','pending batch identities','live edition conflicts'),'transport','authenticated_neon_staged_validated_audited_transaction'),error=null WHERE id=b.id;
END $validate$;
