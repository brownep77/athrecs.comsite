...
assert.equal(upcomingBroadcasts("Cycling", new Date("2026-09-28T02:00:00Z")).length, 1);
assert.equal(upcomingBroadcasts("Cycling", new Date("2026-09-28T05:00:00Z")).length, 0);
assert.equal(upcomingBroadcasts("Triathlon", new Date("2026-09-26T23:00:00Z")).length, 1);
assert.equal(upcomingBroadcasts("Swimming", new Date("2026-10-03T19:59:59Z")).length, 3);
assert.equal(upcomingBroadcasts("Swimming", new Date("2026-10-03T20:00:00Z")).length, 2);
assert.equal(upcomingBroadcasts("Running", new Date("2027-01-01T12:00:00Z")).length, 0);
...
try {
  await db.exec(`
    create table events (id int primary key, name text, sport text, city text, country text, website text);
  ...
  const running = await readSportFixtures(sql, { sport: "Running" }, "2026-09-26");
  assert.deepEqual(
  ...
  assert.equal(
    (await readSportFixtures(sql, { sport: "Running", q: "%" }, "2026-09-26")).fixtures.length,
    1,
  );
  assert.equal(
    (await readSportFixtures(sql, { sport: "Running", q: "' OR 1=1 --" }, "2026-09-26")).fixtures
      .length,
    0,
  );
  const swimming = await readSportFixtures(sql, { sport: "Swimming" }, "2026-09-26");
  assert.equal(swimming.fixtures.length, 1);
  assert.equal(swimming.fixtures[0].website, null);
  assert.equal(
    (await readSportFixtures(sql, { sport: "Cycling" }, "2026-09-26")).fixtures[0].name,
    "Example Ride",
  );
  await db.exec(`
    insert into editions select 100+n, 1, '2026-10-01'::date+n, '10K', null from generate_series(0,30) n;
  `);
  const first = await readSportFixtures(sql, { sport: "Running" }, "2026-09-26");
  const second = await readSportFixtures(sql, { sport: "Running", page: 2 }, "2026-09-26");
