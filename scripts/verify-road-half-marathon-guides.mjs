...
const {
  ROAD_HALF_MARATHONS: ROAD_MARATHONS,
  roadHalfMarathonsForCountry: roadMarathonsForCountry,
} = await import("../src/data/road-half-marathons/index.ts");
const { HALF_MARATHON_COUNTRIES: MARATHON_COUNTRIES } =
  await import("../src/data/road-half-marathons/countries.ts");
assert.equal(
  new Set([...FULL_MARATHONS, ...ROAD_MARATHONS].map((race) => race.slug)).size,
  FULL_MARATHONS.length + ROAD_MARATHONS.length,
  "Half and full races must have separate URLs",
);
...
assert(roadMarathonsForCountry("uk").length > 0);
assert.equal(
...
for (const race of ROAD_MARATHONS) {
  assert.equal(race.distanceKm, 21.0975);
  const graphCheck = JSON.parse(roadRaceHead(race, now).scripts[0].children)["@graph"];
  const countryCrumb = graphCheck.find((item) => item["@type"] === "BreadcrumbList")
    .itemListElement[2];
  assert(countryCrumb.item.endsWith(`/${race.country}-half-marathons`), race.slug);
  assert(countryCrumb.name.includes("half marathons"), race.slug);
