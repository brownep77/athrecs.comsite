...
export async function snapshot(sql: Sql, dates: string[]) {
  ...
  const pending =
    await sql<PendingEdition>`select x->>'eventSlug' as "eventSlug",x->>'date' as date,b.id as "batchId",coalesce(e.name,p->>'name') as name,coalesce(e.country,p->>'country') as country,coalesce(e.city,p->>'city') as city,x->>'source' as source,x->>'entryUrl' as "entryUrl",(x->>'distanceKm')::float8 as "distanceKm" from catalogue_import_batches b cross join lateral jsonb_array_elements(coalesce(b.payload->'editions','[]'::jsonb)) x left join events e on e.slug=x->>'eventSlug' left join lateral jsonb_array_elements(coalesce(b.payload->'events','[]'::jsonb)) p on p->>'slug'=x->>'eventSlug' where b.status not in ('published','rolled_back')`;
