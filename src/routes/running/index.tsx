...
export const Route = createFileRoute("/running/")({
  ...
  head: () => ({
    meta: siteGraphMeta({
      title: "Road Marathon & Half Marathon Guides: Dates, Entry & Results | ATHRECS",
      description:
        "Explore road marathons and half marathons in the UK, Australia, New Zealand, USA, Canada, Ireland and South Africa. Compare entry options, routes, dates and results.",
    ...
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          "@type": "CollectionPage",
          name: "AthRecs road marathon and half marathon guides",
          url: `${SITE_URL}/running`,
          mainEntity: {
            "@type": "ItemList",
            numberOfItems: MARATHON_COUNTRIES.length + HALF_MARATHON_COUNTRIES.length,
            itemListElement: [...MARATHON_COUNTRIES, ...HALF_MARATHON_COUNTRIES].map(
              (country, index) => ({
                "@type": "ListItem",
                position: index + 1,
    ...
    <div className="space-y-8 pb-8">
      <header className="max-w-4xl">
        ...
        <p className="mt-5 max-w-3xl text-base leading-7 text-muted">
          ...
          to explore. Then comes the small matter of training for it.
        </p>
        <p className="mt-4 text-sm font-semibold">
          {ROAD_MARATHONS.length + ROAD_HALF_MARATHONS.length} race guides ·{" "}
          {MARATHON_COUNTRIES.length} countries · Confirmed dates through 2027
        </p>
      </header>
      <nav
