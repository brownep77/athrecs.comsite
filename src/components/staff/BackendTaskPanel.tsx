  ...
  return <section id="backend-tasks" aria-labelledby="backend-tasks-heading" className="mb-10 min-w-0 scroll-mt-6 space-y-5">
    <header className="rounded-2xl border border-cyan-200 bg-white p-5 md:p-7">
      <div className="flex flex-wrap items-start justify-between gap-5">
        ...
        <div className="flex flex-wrap gap-3"><a href="/admin/athlete-workspace" className="inline-flex min-h-11 max-w-full items-center justify-center gap-2 rounded-lg border border-cyan-800 px-4 py-3 text-sm font-semibold text-cyan-900 no-underline">Edit profiles & pasted results<ArrowRight className="size-4 shrink-0" aria-hidden="true" /></a><a href="/admin/check-results-upload" className="inline-flex min-h-11 max-w-full items-center justify-center gap-2 rounded-lg bg-cyan-800 px-4 py-3 text-sm font-semibold text-white no-underline hover:bg-cyan-900 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-800">Import athletes & results<ArrowRight className="size-4 shrink-0" aria-hidden="true" /></a></div>
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="relative max-w-2xl"><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-slate-500" aria-hidden="true" /><input id="backend-task-search" type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Try edit, paste, import, duplicates, claims or sources" className="h-11 w-full min-w-0 rounded-lg border border-slate-300 bg-white pl-10 pr-3 text-sm" /></div>
    ...
    <details className="rounded-xl border border-slate-200 bg-white p-4 text-sm"><summary className="cursor-pointer font-semibold text-slate-700">Pasted results and athlete confirmation</summary><p className="mt-3 leading-6 text-slate-600">The editing workspace accepts common tab-separated or CSV tables, keeps proposals private, records evidence for and against, and creates recipient-bound confirmation links. Missing details and qualification marks need review; copied text is not automatically verified.</p><a className="mt-3 inline-block underline" href="https://www.athrecs.com/athlete-results">Athlete self-service workspace</a></details>
